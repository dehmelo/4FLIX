...
def filme_especifico(id):
    ...
    for i in info:
        ...
        if 'comentarios' in i.keys():
            i['comentarios'] = [{"nome": c.get("nome"), "comentario": c.get("comentario")} for c in i['comentarios']]
        dic['info'].append(i)
...
def ver_filme(id):
    info = db.filmes.update_one({"_id": ObjectId(id)}, {"$set": {"visto": datetime.datetime.now()}})
...
def comentario(id):
    ...
    info = db.filmes.update_one(
        {'_id': ObjectId(id)},
        {'$addToSet': {'comentarios': comment.inserted_id}}
    )
...
def nota(id):  
    ...
    result = db.filmes.update_one({"_id": ObjectId(id)}, {"$inc": {"avaliacao.%s" % nota: 1}})
    return jsonify({"message": "Comentario enviado"})
...
def novo_filme_geral():
    ...
    filme['avaliacao'] = {'bom': 0, 'medio': 0, 'ruim': 0}
    filme['arquivo'] = 'static/img/ghost-in-the-shell.jpg'
    filme = db.filmes.insert_one(filme)
...
def ver_recentes():
    filmes = db.filmes.find({}, {"titulo": 1, "visto": 1, "_id": 0}).sort([('visto', pymongo.DESCENDING)]).limit(4)
...
def ver_recomendados():
    filmes = db.filmes.find({}, {"titulo": 1, "avaliacao": 1, "_id": 0}).sort([('avaliacao.bom', pymongo.DESCENDING)]).limit(4)
    dic = {'filmes': [f for f in filmes]}
    print(dic)
    return jsonify(dic)
